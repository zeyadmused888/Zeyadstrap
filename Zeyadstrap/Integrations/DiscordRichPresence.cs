    ...
    public class DiscordRichPresence : IDisposable
    {
        ...
        public DiscordRichPresence(ActivityWatcher activityWatcher)
        {
            ...
            _rpcClient.OnConnectionFailed += (_, e) =>
                App.Logger.WriteLine(LOG_IDENT, $"Failed to establish connection with Discord RPC ({e.FailedPipe})");
            ...
            _refreshTimer = new Timer(_ => RefreshPresence(), null, TimeSpan.FromSeconds(15), TimeSpan.FromSeconds(15));
        }
        ...
        private void SetBrowsingRobloxAppPresence()
        {
            _currentPresence = new DiscordRPC.RichPresence
            {
                Details = "Browsing Roblox",
                State = "Using Zeyadstrap",
        ...
        public void UpdatePresence(bool refresh = false)
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, "RPC client is not configured, skipping presence update");
                return;
            }

            if (!_ready)
            {
                App.Logger.WriteLine(LOG_IDENT, "RPC client is not ready, deferring presence update");
                return;
            }
        ...
        {
            if (_currentPresence is not null && _visible)
                UpdatePresence(true);
        }
        private void ClearPresence()
        {
            const string LOG_IDENT = "DiscordRichPresence::ClearPresence";

...
using Zeyadstrap.UI.Elements.Dialogs;
using Zeyadstrap.Enums;
...
namespace Zeyadstrap
{
    ...
    {
        private static LauncherRichPresence? _launcherRichPresence;

        public static void SetLauncherRichPresence(string details, string? state = null) => _launcherRichPresence?.SetActivity(details, state);
        ...
        public static void LaunchSettings()
        {
            ...
            _launcherRichPresence?.Dispose();
            _launcherRichPresence = LauncherRichPresence.TryCreate("In settings", "Configuring Zeyadstrap");
        ...
        {
            _launcherRichPresence?.Dispose();
            using var richPresence = LauncherRichPresence.TryCreate("In the launcher", "Choosing what to open");
        ...
        public static void LaunchRoblox(LaunchMode launchMode)
        {
            ...
            _launcherRichPresence?.Dispose();
            _launcherRichPresence = LauncherRichPresence.TryCreate(
                GetRobloxPresenceDetails(launchMode),
                "Using Zeyadstrap");
        ...
        private static string GetRobloxPresenceDetails(LaunchMode launchMode)
        {
            if (launchMode == LaunchMode.Studio || launchMode == LaunchMode.StudioAuth)
                return "Launching Roblox Studio";

            return String.IsNullOrWhiteSpace(App.LaunchSettings.RobloxLaunchArgs)
                ? "Browsing Roblox"
                : "Launching Roblox";
        }
